...
| [`builder.html`](builder.html) | **機體卡產生器（電腦版）**：拖拉編組、即時計價、列印 A4 機體卡、複製分享連結 |
| [`builder-mobile.html`](builder-mobile.html) | **機體卡產生器（手機版）**：改用點選式（tap-to-pick）＋單欄版面，觸控可用；資料與列印卡與電腦版互通 |
| [`helper.html`](helper.html) | **對戰 Helper**：《玩家操作 SOP》的可執行版，算骰數門檻、維護我方熱／結構／時間軌、攻守結算交握 |

> 入口頁會依裝置自動把「機體卡產生器」指向電腦版（拖拉）或手機版（點選）。手機版與電腦版**共用同一份 `DATA`**——改武器/數值兩個檔都要改。
